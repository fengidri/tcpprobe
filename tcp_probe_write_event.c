...
#define FLAG_RECV 1
#define FLAG_EVENT_WRITE 2

const char * flag_string[] = {
    "Recv Packet", "Write Space"
};
...
static void jsk_stream_write_space(struct sock *sk)
{
	const struct tcp_sock *tp = tcp_sk(sk);
	const struct inet_sock *inet = inet_sk(sk);
...
static struct jprobe tcp_jwrite_event = {
	.kp = {
		.symbol_name	= "sk_stream_write_space",
	},
	.entry	= jsk_stream_write_space,
};
...
static __init int tcpprobe_init(void)
{
	...
	if (ret)
		goto err1;
	ret = register_jprobe(&tcp_jwrite_event);
	if (ret)
		goto err1;
...
static __exit void tcpprobe_exit(void)
{
	...
	unregister_jprobe(&tcp_jwrite_event);
	kfree(tcp_probe.log);
}
